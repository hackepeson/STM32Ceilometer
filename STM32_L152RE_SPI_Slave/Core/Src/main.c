...
/* USER CODE BEGIN PV */
uint16_t spi_tx_data[100];
uint16_t spi_rx_data[100];
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1) {
		 ...
		 {
		 //sprintf(txBuf,"0\n");
		 //HAL_UART_Transmit(&huart2,txBuf,strlen(txBuf),10);
		 }

/*
		spi_error = HAL_SPI_TransmitReceive_DMA(&hspi2, (uint8_t*) spi_tx_data,
				(uint8_t*) spi_rx_data, sizeof(spi_tx_data) / 2);
		if (spi_error == HAL_OK)
		{
			sprintf(txBuf, "%x\n", spi_rx_data[0]);
			HAL_UART_Transmit(&huart2, (uint8_t*)txBuf, strlen(txBuf), 10);
...
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi) {
	...
	spi_error = HAL_SPI_TransmitReceive_DMA(&hspi2,			(uint8_t*) spi_tx_data, (uint8_t*) spi_rx_data,	sizeof(spi_tx_data)/2);
	spi_error = HAL_SPI_TransmitReceive_IT(&hspi2,
			(uint8_t*) spi_tx_data, (uint8_t*) spi_rx_data,
			sizeof(spi_tx_data)/2);
